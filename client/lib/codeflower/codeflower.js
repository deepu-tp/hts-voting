CodeFlower = function(selector, w, h) {
  this.w = w;
  this.h = h;
  this.padding = 10;
  ...
  this.force = d3.layout.force()
    ...
    .charge(function(d) { return d._children ? -d.size * 3 / 100 : -40; })
    .linkDistance(function(d) { return (d.target._children || d.target.children) ? 150 : 5; })
    .size([h, w]);
}
...
CodeFlower.prototype.update = function(json) {
  ...
  d3.select('body').selectAll(".cf-tooltip").remove();
  ...
  this.node.transition()
    .attr("r", function(d) { return d.children ? 3.5 : d.size * 3 || 3.5; });
  ...
  this.node.enter().append('svg:circle')
    ...
    .style("fill", function color(d) {
      return (d._children || d.children) ? d.color : 'black';
    })
    .style("opacity", 0.8)
    .style("stroke", function color(d) {
      return (d._children || d.children) ? "black" : d.color;
    })
...
CodeFlower.prototype.mouseover = function(d) {
  console.log(d)
  var vtext = " (" + d.size + (d.size == 1 ? " vote)" : " votes)")
  var ctext = " : " + d.comment
  var text = d.display_name + ((d._children || d.children) ? vtext : ctext)
